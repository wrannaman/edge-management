...
class IntegrationDownshift extends React.Component {
  ...
  onSelect = (name) => {
    ...
    // update('moveTo', found);
    // console.log('moveTo', found)
  }

  getLabels = (boxes) => boxes.slice().map(b => ({ uuid: b.uuid, label: b.label ? b.label : "" }))
                ...
                <div {...getMenuProps()}>
                  {isOpen ? (
                    <Paper className={classes.paper} square>
                      {getSuggestions(getLabels(boxes), inputValue).map((suggestion, index) =>
                        renderSuggestion({
                          suggestion,
                          index,
